...
try:
    ...
    df.rename(columns={'Taxa': 'taxa'}, inplace=True)
...
def apply_filters(
    ...
    if english_name: query_df = query_df[query_df['english_name'].isin(english_name.split(','))]
    if species: query_df = query_df[query_df['species'].isin(species.split(','))]
    if obs: query_df = query_df[query_df['obs'].isin(obs.split(','))]
    if taxa: query_df = query_df[query_df['taxa'].isin(taxa.split(','))]
    if year: query_df = query_df[query_df['year'] == int(year)]
    if month: query_df = query_df[query_df['month'] == int(month)]
    return query_df
...
def get_filter_options(english_name: Optional[str] = None, species: Optional[str] = None, obs: Optional[str] = None, taxa: Optional[str] = None, year: Optional[str] = None, month: Optional[str] = None):
    base_df = df
    def get_options(df_source, key_name):
    ...
    options['english_name'] = get_options(temp_df, 'english_name')
    temp_df = apply_filters(base_df, english_name=english_name, obs=obs, taxa=taxa, year=year, month=month)
    options['species'] = get_options(temp_df, 'species')
    temp_df = apply_filters(base_df, english_name=english_name, species=species, taxa=taxa, year=year, month=month)
    options['obs'] = get_options(temp_df, 'obs')
    temp_df = apply_filters(base_df, english_name=english_name, species=species, obs=obs, year=year, month=month)
    options['taxa'] = get_options(temp_df, 'taxa')
    temp_df = apply_filters(base_df, english_name=english_name, species=species, obs=obs, taxa=taxa, month=month)
    options['year'] = get_options(temp_df, 'year')
    temp_df = apply_filters(base_df, english_name=english_name, species=species, obs=obs, taxa=taxa, year=year)
    options['month'] = get_options(temp_df, 'month')
    return options

@app.get("/api/records")
def get_records(page: int = 1, page_size: int = 100, english_name: Optional[str] = None, species: Optional[str] = None, obs: Optional[str] = None, taxa: Optional[str] = None, year: Optional[int] = None, month: Optional[int] = None):
    query_df = apply_filters(df.copy(), english_name, species, obs, taxa, year, month)
    total_records = len(query_df)
    paginated_data = query_df.iloc[(page - 1) * page_size:page * page_size]
    paginated_data = paginated_data.replace([np.inf, -np.inf], None).astype(object).where(pd.notnull(paginated_data), None)
    return jsonable_encoder({
        "total_records": total_records, "page": page,
        "total_pages": ceil(total_records / page_size),
        "records": paginated_data.to_dict(orient='records')
    })

@app.get("/api/summary/diversity")
def get_diversity_summary(english_name: Optional[str] = None, species: Optional[str] = None, obs: Optional[str] = None, taxa: Optional[str] = None, year: Optional[int] = None, month: Optional[int] = None):
    from scipy.stats import entropy
    query_df = apply_filters(df.copy(), english_name, species, obs, taxa, year, month)
    species_counts = query_df.groupby('species')['count'].sum()
    ...
    return {
        "shannon": round(shannon_index, 3),
        "simpson": round(gini_simpson_index, 3),
        "species_richness": int(species_richness)
    }


@app.get("/api/summary/species_distribution")
def get_species_distribution(english_name: Optional[str] = None, species: Optional[str] = None, obs: Optional[str] = None, taxa: Optional[str] = None, year: Optional[int] = None, month: Optional[int] = None):
    query_df = apply_filters(df.copy(), english_name, species, obs, taxa, year, month)
    if query_df.empty:
        return []
...
def get_temporal_trends(english_name: Optional[str] = None, species: Optional[str] = None, obs: Optional[str] = None, taxa: Optional[str] = None, year: Optional[int] = None, month: Optional[int] = None):
    query_df = apply_filters(df.copy(), english_name, species, obs, taxa, year, month)
    if query_df.empty: return {}
    summary = query_df.groupby('month').size().reindex(range(1, 13), fill_value=0)
    return summary.to_dict()

@app.get("/api/summary/observer_comparison")
def get_observer_comparison(english_name: Optional[str] = None, species: Optional[str] = None, obs: Optional[str] = None, taxa: Optional[str] = None, year: Optional[int] = None, month: Optional[int] = None):
    if not obs: return {}
...
def get_observer_stats(observer_name: str, english_name: Optional[str] = None, species: Optional[str] = None, taxa: Optional[str] = None, year: Optional[int] = None, month: Optional[int] = None):
